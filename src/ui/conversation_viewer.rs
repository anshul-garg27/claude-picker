...
/// Zen-aware render entry point.
///
/// When `zen == true` we drop every bit of chrome — bordered block, title
/// line, footer, search bar, and the interesting-moments mini-timeline —
/// so only the transcript body survives. When `zen == false` the
/// interesting-moments mini-timeline is prepended above the transcript
/// body so the reader can see where the notable events (cost spikes, tool
/// bursts, long pauses) are concentrated. Every other caller should use
/// [`render`] which keeps the chrome-on default.
