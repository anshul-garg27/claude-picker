...
pub fn render(frame: &mut Frame<'_>, area: Rect, view: &StatsView<'_>, theme: &Theme) {
    ...
    let rows = Layout::default()
        ...
        .constraints([
            ...
            Constraint::Length(8),              // 2  kpi cards (rich)
            Constraint::Length(1),              // 3  blank
            Constraint::Min(8),                 // 4  per-project + quota + activity + patterns flex
            Constraint::Length(project_heat_h), // 5  FEAT-6: project 30d heatmap
            Constraint::Length(hist_h),         // 6  turn-duration histogram
            Constraint::Length(burn_h),         // 7  burn-rate alert (#12)
            Constraint::Length(budget_h),       // 8  budget band
            Constraint::Length(by_model_h),     // 9  by-model pills
    ...
    if burn_h > 0 {
        render_burn_alert(frame, rows[7], view, theme);
    }
    if budget_h > 0 {
        render_budget_band(frame, rows[8], view, theme);
    }
    if by_model_h > 0 {
        render_by_model(frame, rows[9], view, theme);
    }
    render_footer(frame, rows[10], theme);
